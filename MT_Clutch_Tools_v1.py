...
grep -a "final result" %%%input_string%%%.out > %%%input_string%%%_done.dat"""        
        ...
        for location in sdf_dicts[0]:
            ...
            for isotope in sdf_dicts[0][location]:
                ...
                for sdf_count, sdf_dict in enumerate(sdf_dicts):
                    ...
                    combined_dict[location][isotope]['sensitivity'] += float(sdf_dict[location][isotope]['sensitivity']) * int(generation_list[sdf_count])/summed_generation_list
                    combined_dict[location][isotope]['uncertainty'] += float(sdf_dict[location][isotope]['uncertainty']) * int(generation_list[sdf_count])/summed_generation_list
    ...
    def build_scale_input_from_beta(self,
        ...
        for count, material in enumerate(material_list):
            material_string_list.append(self.sfh.build_scale_material_string(material, count + material_count_offset, temperature ))
